...
void DBDetector::WriteData(std::string data, std::string saveFile)
{
    ofstream outfile(saveFile, ios::out);
    outfile << data;
    outfile.close();
}
...
void DBDetector::Run(cv::Mat& img, std::string& detresult) {
    float ratio_h{};
    float ratio_w{};
    std::vector<std::vector<std::vector<int>>> boxes;
    cv::Mat srcimg;
    ...
    for (int n = 0; n < boxes.size(); n++) {
        for (int m = 0; m < boxes[n].size(); m++) {
            one_box.clear();
            one_box.str("");
            one_box <<int(boxes[n][m][0]) << "," << int(boxes[n][m][1]) << ";";
            detresult.append(one_box.str());
          
        }
    }
    //detresult = outstr;
    //ofstream outfile("D:/result.txt", ios::out);
    //outfile << outstr;
    //outfile.close();
}
} // namespace PaddleOCR
